...
function normalizeState(input) {
  const hasCurrentPlan = Array.isArray(input.plan) && Number(input.planVersion) >= 3;
  return {
...
function renderStretches() {
  renderStretchList("warmupList", activeDay().warmup, "warmup");
  renderStretchList("cooldownList", activeDay().cooldown, "cooldown");
}

function renderStretchList(id, stretches, kind) {
  const list = $(id);
  list.innerHTML = "";
  stretches.forEach(([name, cue], index) => {
    const card = document.createElement("article");
    card.className = "stretch-card";
    card.innerHTML = `
      <input data-field="stretchName" data-index="${index}" type="text" value="${escapeAttr(name)}" aria-label="Stretch name" />
      <input data-field="stretchCue" data-index="${index}" type="text" value="${escapeAttr(cue)}" aria-label="Stretch cue" />
      <button class="text-button danger" data-action="removeStretch" data-index="${index}" type="button">Remove</button>
      <a class="video-link" href="${stretchVideoUrl(name)}" target="_blank" rel="noopener noreferrer">Watch demo</a>
    `;
    card.querySelectorAll("input").forEach((input) => {
      input.addEventListener("input", () => {
        const stretch = activeDay()[kind][Number(input.dataset.index)];
        stretch[input.dataset.field === "stretchName" ? 0 : 1] = input.value;
        saveState();
      });
    });
    card.querySelector('[data-action="removeStretch"]').addEventListener("click", () => {
      activeDay()[kind].splice(index, 1);
      render();
    });
  ...
  form.className = "add-stretch-form";
  form.innerHTML = `
    <input data-field="newStretchName" type="text" maxlength="48" placeholder="Add stretch" />
    <input data-field="newStretchCue" type="text" maxlength="80" placeholder="Cue or duration" />
    <button class="text-button" type="button">Add</button>
  `;
  form.querySelector("button").addEventListener("click", () => addStretch(kind, form));
  list.appendChild(form);
}

function addStretch(kind, form) {
  const name = form.querySelector('[data-field="newStretchName"]').value.trim();
  const cue = form.querySelector('[data-field="newStretchCue"]').value.trim() || "30 seconds";
  if (!name) return;
  activeDay()[kind].push([name, cue]);
  render();
}
...
function renderLifts() {
  const list = $("liftList");
  list.innerHTML = "";
  renderAddLiftForm(list);
  activeDay().lifts.forEach((item) => {
    syncLiftSetCount(item);
    const previous = previousLiftResult(item.name);
    const target = targetForLift(item, previous);
    const card = document.createElement("article");
    ...
    card.innerHTML = `
      <div class="lift-title-row">
        ...
        <div class="lift-actions">
          ...
          <button class="text-button danger" data-action="deleteLift" type="button">Delete</button>
        </div>
      </div>
      <div class="progress-hint">
        <span>${previous ? `Previous: Week ${previous.weekNumber}, ${previous.date} - ${escapeHtml(previous.summary)}` : "Previous: no saved result yet"}</span>
          ...
          .map(
            (entry, index) => `
              <div class="set-row">
                ...
                <button class="mini-button danger" data-action="removeSet" data-set="${index}" type="button">-</button>
              </div>
            `,
          )
          .join("")}
      </div>
      <button class="text-button add-set-button" data-action="addSet" type="button">Add set</button>
    ...
    card.querySelector('[data-action="deleteLift"]').addEventListener("click", () => deleteLift(item.id));
    card.querySelector('[data-action="addSet"]').addEventListener("click", () => addSet(item.id));
    card.querySelectorAll('[data-action="removeSet"]').forEach((button) => {
      button.addEventListener("click", () => removeSet(item.id, Number(button.dataset.set)));
    });
    card.querySelectorAll("input").forEach((input) => {
      input.addEventListener("input", () => {
...
function renderAddLiftForm(list) {
  const form = document.createElement("section");
  form.className = "add-lift-form";
  form.innerHTML = `
    <div>
      <h3>Add Lift To This Workout</h3>
      <p>Add one-off lifts here. Use Swap to save reusable lift options.</p>
    </div>
    <div class="add-lift-grid">
      <input id="newLiftName" type="text" maxlength="48" placeholder="Lift name" />
      <select id="newLiftCategory">
        ${Object.entries(categoryLabels).map(([value, label]) => `<option value="${value}">${label}</option>`).join("")}
      </select>
      <input id="newLiftMuscles" type="text" maxlength="80" placeholder="Targets, comma separated" />
      <input id="newLiftSets" type="number" min="1" max="8" step="1" placeholder="Sets" />
      <input id="newLiftReps" type="text" maxlength="20" placeholder="Reps" />
    </div>
    <button class="text-button" type="button">Add lift</button>
  `;
  form.querySelector("button").addEventListener("click", () => addLiftToWorkout(form));
  list.appendChild(form);
}

function addLiftToWorkout(form) {
  const name = form.querySelector("#newLiftName").value.trim();
  if (!name) return;
  const category = form.querySelector("#newLiftCategory").value;
  const muscles = form
    .querySelector("#newLiftMuscles")
    .value.split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  const sets = form.querySelector("#newLiftSets").value || "3";
  const reps = form.querySelector("#newLiftReps").value.trim() || "10-12";
  activeDay().lifts.push(lift(name, category, muscles.length ? muscles : [categoryLabels[category]], sets, reps));
  render();
}

function deleteLift(liftId) {
  const day = activeDay();
  day.lifts = day.lifts.filter((item) => item.id !== liftId);
  render();
}

function addSet(liftId) {
  const item = activeDay().lifts.find((liftItem) => liftItem.id === liftId);
  if (!item) return;
  item.entries.push({ weight: "", reps: "", note: "" });
  syncLiftSetCount(item);
  render();
}

function removeSet(liftId, setIndex) {
  const item = activeDay().lifts.find((liftItem) => liftItem.id === liftId);
  if (!item || item.entries.length <= 1) return;
  item.entries.splice(setIndex, 1);
  syncLiftSetCount(item);
  render();
}

function syncLiftSetCount(item) {
  item.sets = String(item.entries.length || Number(item.sets) || 1);
}

function previousLiftResult(name) {
  const sessions = allCompletedSessions();
  const match = sessions.find((session) => session.lifts?.some((item) => item.name === name));
  if (!match) return null;
  const liftEntry = match.lifts.find((item) => item.name === name);
  const best = bestSet(liftEntry.entries);
  return {
    weekNumber: match.weekNumber,
    date: match.date,
    summary: best ? `${best.weight} x ${best.reps}` : "saved with no set data",
    best,
  };
}

function allCompletedSessions() {
  const current = sessionsForWeek(state.currentWeek);
  const archived = [...state.weeks].reverse().flatMap((week) => sessionsForWeek(week).reverse());
  return [...current.reverse(), ...archived];
}

function bestSet(entries = []) {
  return entries
    .map((entry) => ({ weight: Number(entry.weight), reps: Number.parseInt(entry.reps, 10) }))
    .filter((entry) => Number.isFinite(entry.weight) && entry.weight > 0 && Number.isFinite(entry.reps) && entry.reps > 0)
    .sort((a, b) => b.weight * b.reps - a.weight * a.reps)[0] || null;
}

function targetForLift(item, previous) {
  if (!previous?.best) return "Target: log this lift once";
  const repGoal = topRepGoal(item.reps);
  if (previous.best.reps >= repGoal) {
    return `Target: ${previous.best.weight + weightJump(previous.best.weight)} x ${Math.max(1, repGoal - 2)}`;
  }
  return `Target: ${previous.best.weight} x ${previous.best.reps + 1}`;
}

function topRepGoal(repText) {
  const matches = String(repText).match(/\d+/g);
  if (!matches?.length) return 12;
  return Math.max(...matches.map(Number));
}

function weightJump(weight) {
